            ...
            if current_time == "13:16:00":
                ...
                for i in known_face_names:
                    if i not in jk:
                        notpresent.append(i)
                print(notpresent)
    def mailing(notpresent):
        mailid = []

        for j in notpresent:
            mycursor = mydb.cursor(buffered=True)
            # sqlform = "insert into STUDENTS(Name, Rollno, Department, Barcodid, ParentsMailId) Values(%s,%s,%s,%s,%s)"
            # STUDENTS = [("Abhijith Jayan", 2, "BCA", 5468, "abhijithjay777@gmail.com"), ("Ajay J Kumar", 8, "BCA", 3232, "ajayjayakumar2000@gmail.com"),("Chanthukrishnan J", 24, "BCA", 2255, "chanthukrishnan69@gmail.com"),("Karthik P", 38, "BCA", 6325, "karthikadi2001@gmail.com"),("R Padmanabhan", 53, "BCA", 8562, "padram212@gmail.com")]
            # mycursor.executemany(sqlform, STUDENTS)
            selectQuery = "SELECT ParentsMailId FROM STUDENTS where Name = " + j + ";"
            mycursor.execute(selectQuery)
            mailid.append(selectQuery)
        if current_time == "13:18:00":
            for k in mailid:
                print(k)
